...
import ActionBar, { Item as ActionItem } from "../../components/ActionBar/ActionBar";
...
export default function DataPage() {
  ...
  const valueColor = (datasetName, value) => {
    ...
    if (datasetName.includes("Temperature")) return value >= 1.0 ? "#b91c1c" : "#ca8a04";
    if (datasetName.includes("Arctic Sea Ice")) return value <= 4.0 ? "#b91c1c" : "#1d4ed8";
    if (datasetName.includes("Antarctic Sea Ice")) return value <= 18.0 ? "#b91c1c" : "#1d4ed8";
    return "#374151";
  };
  ...
  return (
    <MaxWidth>
      <section className="data" id="dashboard">
        ...
        {/* buttons laid out by ActionBar (no change to Button component) */}
        <ActionBar>
            <ActionItem><Button onClick={() => setView("all")}>Show All (table)</Button></ActionItem>
            <ActionItem><Button onClick={() => setView("latest")}>Latest Year</Button></ActionItem>
            <ActionItem><Button onClick={() => setView("byDataset")}>By Dataset</Button></ActionItem>
            <ActionItem><Button onClick={() => setView("extremes")}>Extremes</Button></ActionItem>
            <ActionItem><Button onClick={() => setView("trends")}>Trends</Button></ActionItem>
            <ActionItem><Button onClick={() => setView("summary")}>Summary</Button></ActionItem>
            <ActionItem><Button onClick={reset} variant="secondary">Reset Filters</Button></ActionItem>
        </ActionBar>
              ...
              <div className="groups">
                {byDataset.map((g) => (
                  <div key={g.dataset_id} className="group">
                    ...
                    <div className="chips">
                      {g.values.slice().sort((a,b)=>a.year-b.year||a.month-b.month).map((v) => (
                        <span key={`${g.dataset_id}-${v.year}-${v.month}`}
                              className="chip"
                              style={{
                                background: "#f3f4f6",
              ...
              <ul className="trendlist">
                {trendRows.map((t) => (
                  <li key={t.dataset}>
                    ...
                    <span style={{
                      color: valueColor(t.dataset, t.delta >= 0 ? t.last.value : t.first.value),
                    }}>
